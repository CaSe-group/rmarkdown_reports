...
`r if(nrow(abricate_input)>0){ sprintf(The sample **`r sample_name`** contains **`r nrow(abricate_input)` resistance genes**.)} 

`r if(nrow(abricate_input)>0){ sprintf(Determined by ABRicate using the **`r if(nrow(abricate_input)<1){database_name [!duplicated(database_name),]}`** Database.)}
