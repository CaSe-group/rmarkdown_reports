### Prokka

```{r setup, include=FALSE} 
knitr::opts_chunk$set(warning = FALSE, message = FALSE, echo=FALSE) 
```
```{r libraries, echo=FALSE}
library(ggplot2)
library(dplyr)
library(tidyr)
library(readr)
library(stringr)
library(DT)
library(plotly)
```

```{r data_read, echo=FALSE}
PROKKA_INPUT_FILE <- "#RESULTSENV#"
PROKKA_VERSION <- "#NAMEENV#"

PROKKA_RAW <- read_delim(PROKKA_INPUT_FILE, 
    delim = "\t", escape_double = FALSE,
    col_names = FALSE, comment = c("#"), trim_ws = TRUE)

PROKKA_RAW <- na.omit(PROKKA_RAW)

PROKKA_PROC <- separate(PROKKA_RAW, X9, c(NA, "name"), remove = FALSE, "Name=")
PROKKA_PROC$name <- gsub(";.*","",PROKKA_PROC$name)
```

```{r text_parsing}
GEN_N <- PROKKA_PROC %>% filter(X3 != 'CDS') %>% count() %>% as.integer()
GEN_N_HYP <- PROKKA_PROC %>% filter(X3 != 'CDS' & name !='NA') %>% count() %>% as.integer()
GEN_N_TRUE <- GEN_N-GEN_N_HYP
HYP_PERC <- round(GEN_N_HYP/GEN_N*100, 1)
CONTIG_N <- length(unique(PROKKA_PROC$X1))
RRNA_N_GEN <- PROKKA_PROC %>% filter(X3 =="rRNA") %>% count() %>% as.integer()
TRNA_N_GEN <- PROKKA_PROC %>% filter(X3 =="tRNA") %>% count() %>% as.integer()
PIE <- data.frame(n=c(GEN_N_TRUE,GEN_N_HYP,RRNA_N_GEN,TRNA_N_GEN ),Type=as.factor(c("Genes","Hypothetical Genes", "rRNA", "tRNA")))
```
Prokka is a software tool to annotate bacterial, archaeal and viral genomes quickly and produce standards-compliant output files. 
[Prokka Github](https://github.com/tseemann/prokka)

Prokka founded **`r GEN_N` genes**, **`r GEN_N_HYP` were hypothetical** <span style="color:red; font-weight: bold">
`r if(GEN_N_HYP/GEN_N > 0.7){
"Too many hypothetical genes!"
}` </span> Genese were located on **`r CONTIG_N` contigs**.

There were **`r RRNA_N_GEN` rRNA** genes <span style="color:red; font-weight: bold">`r if (RRNA_N_GEN > 3){
"You have more than expected rRNA sequences for one species!"
}`</span> and **`r TRNA_N_GEN` tRNA** genes.
```{r comp_plot}
#margins
M <- list(
          l = 50,
          r = 50,
          b = 50,
          t = 50,
          pad = 4
        )

COMP_FIG <- plot_ly(PIE, labels = ~Type, values = ~n, width = 450, height = 450,
    marker = list(colors=c("#008b8b", "#ff8c00", "#9067a7", 
                           "#dc143c", "#00bfff"))) %>% 
    add_pie(hole = 0.6) %>% layout(showlegend = TRUE,
                                    margin = M,
                                    legend = list(orientation = 'h'),
                                  xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
                                  yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))

COMP_FIG
```
Genes denotes genes that Prokka found in its databases, hypothetical genes are potential genes that were predicted but could not be found at the database, tRNA and rRNA denotes genes that were attributed to tRNA and rRNA genes. 
Clicking on the legend will exclude the feature from the composition and clicking in the right corner on camera allows to download currently displayed plot as a png file.  

#### **Composition and contig length plots**
Visualisation of features found on contigs compared with length of contigs.

```{r data_for_plots}
#### Data conversion 

CONT_LEN <- PROKKA_PROC %>% select(X1, X5) %>% group_by(X1) %>% summarise(
                                                                          cont_max = max(X5)
                                                                          )

CONT_COMP <- PROKKA_PROC %>% select(X1, X3, name)
CONT_COMP$name[!is.na(CONT_COMP$name)] <- "Gene"
CONT_COMP$name[is.na(CONT_COMP$name)] <- "HypotheticalGene"
CONT_COMP$name[str_detect(CONT_COMP$X3,"rRNA")==TRUE] <- "rRNA" #database name for rRNA from Prokka
CONT_COMP$name[str_detect(CONT_COMP$X3,"tRNA")==TRUE] <- "tRNA" #database name for tRNA from Prokka

CONT_TALLY <- CONT_COMP %>% select(X1, name) %>% count(X1,name)
CONT_TALLY <-spread(CONT_TALLY, key = "name", value = "n")
CONT_TALLY <- merge(CONT_LEN,CONT_TALLY, by="X1" )
CONT_TALLY <- CONT_TALLY %>% arrange(desc(cont_max))

#### Fig dimensions
len <- nrow(CONT_LEN)
if(len < 9){
len <- 2
}

dimension <- len * 0.14

if(dimension < 2){
dimension <- 4
}
```
```{r contig_plots, fig.width = dimension, fig.height = dimension}
#### Plots 
#### Comp Plot
CONT_COMP_FIG <- plot_ly(CONT_TALLY, y = ~X1, x = ~Gene, type = 'bar', orientation = 'h',
        marker = list(color = 'rgb(0, 139, 139)'),
                      text= CONT_TALLY$n.Gene,textposition = 'inside', name = 'Gene',
        hovertemplate = paste('(%{x}, %{y})'))
CONT_COMP_FIG <- CONT_COMP_FIG %>% add_trace(x = ~HypotheticalGene, marker = list(color = 'rgb(255, 140, 0)'),
                         text= CONT_TALLY$n.HypotheticalGene,textposition = 'inside', name = 'Hypothetical Gene')
CONT_COMP_FIG <- CONT_COMP_FIG %>% add_trace(x = ~rRNA, marker = list(color = 'rgb(220,20,60)'),
                         text= CONT_TALLY$n.rRNA,textposition = 'inside', name = 'rRNA') 
CONT_COMP_FIG <- CONT_COMP_FIG %>% add_trace(x = ~tRNA, marker = list(color = 'rgb(0,191,255)'),
                         text= CONT_TALLY$n.tRNA,textposition = 'inside', name = 'tRNA')

CONT_COMP_FIG <- CONT_COMP_FIG %>% layout(xaxis = list(title = "Gene count",
                                                        showgrid = TRUE,
                                                        showline = FALSE,
                                                        showticklabels = TRUE,
                                                        zeroline = FALSE,
                                                        domain = c(0.15, 1)),
                                                        yaxis = list(title = "Contigs",
                                                        categoryorder= 'total ascending',
                                                        showgrid = FALSE,
                                                        showline = FALSE,
                                                        showticklabels = TRUE,
                                                        zeroline = FALSE
                                                        ),
                                           barmode = 'stack',
                                           paper_bgcolor = 'rgb(255, 255, 255)', plot_bgcolor = 'rgb(255, 255, 255)',
                                           showlegend = TRUE) 


#### Length plot
CONT_LENGTH_FIG <- plot_ly(x = CONT_TALLY$cont_max, y = CONT_TALLY$X1, name = 'Contig length',
              type = 'bar', mode = 'markers',
              line = list(color = 'rgb(128, 0, 128)'),
              text=CONT_TALLY$cont_max, textposition = 'auto',
              hovertemplate = paste('(%{x}, %{y})')
              ) 
CONT_LENGTH_FIG <- CONT_LENGTH_FIG %>% layout(yaxis = list(
                      categoryorder= 'total ascending',
                      showgrid = TRUE, 
                      showline = TRUE, 
                      showticklabels = FALSE,
                      linecolor = 'rgb(102, 102, 102, 0.8)', 
                      domain = c(0.15, 1)),
                      xaxis = list(title = "Contig length",
                      zeroline = FALSE, 
                      showline = FALSE, 
                      showticklabels = TRUE, 
                      showgrid = TRUE,
                      side = 'down'),
         showlegend = FALSE) 

COMBINED_CONT_FIG <- subplot(CONT_COMP_FIG, CONT_LENGTH_FIG, shareY = TRUE, titleX = TRUE) 
COMBINED_CONT_FIG
```
Pointing a bar will display more details about currently shown element. The plot is zoomable by highlighting a region that you want to zoom into. Clicking at the right corner on camera allows to download currently displayed plot as a png file.

#### **Output table**
Full Prokka output table presented from GFF file with added "Name" column for gene names, extracted from "Attributes" column. For full description of columns please look at [link](https://en.wikipedia.org/wiki/General_feature_format)
```{r prokka_table}

DTABLE <- datatable(PROKKA_PROC, colnames = c("Contig", "Source", "Feature", "Start", "End","Score","Strand","Phase", "Attributes", "Name"),  rownames = FALSE, class = 'hover', extensions  = 'Buttons', options = list(
    dom = 'Bfrtip', buttons = c('copy', 'csv', 'excel', 'pdf', 'colvis')))

DTABLE
#saveWidget(DTABLE, "DTABLE.html")
```
The table is fully interactive and searchable. It is possible to select shown columns by using "Column visiblity" tab. It is possible to download currently displayed table to a clipboard, csv, xlsx or pdf depending on user preferences.

#### **Version**

Prokka Version: **`r PROKKA_VERSION`**


Citation for Prokka can be found under [DOI: 10.1093/bioinformatics/btu153](https://doi.org/10.1093/bioinformatics/btu153).
