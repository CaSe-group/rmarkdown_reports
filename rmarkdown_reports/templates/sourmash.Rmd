...
if (METAGENOMIC_DATA_EXIST) {
	F_MATCH_VALUE <- NA
	if (!is.na(METAGENOMIC_DATA$f_match[1]) & !is.na(METAGENOMIC_DATA$name[1])) {
		METAGENOMIC_DATA_NOT_EMPTY <- TRUE
		if (CLASSIFICATION_DATA_EXIST) {
			F_MATCH_VALUE <- ifelse(!is.na(SOURMASH_SPECIES) & any(grepl(SOURMASH_SPECIES, METAGENOMIC_DATA$name)), round(METAGENOMIC_DATA$f_match[grep(SOURMASH_SPECIES, METAGENOMIC_DATA$name)],2),
					ifelse(!is.na(SOURMASH_GENUS) & any(grepl(SOURMASH_GENUS, METAGENOMIC_DATA$name)), round(METAGENOMIC_DATA$f_match[grep(SOURMASH_GENUS, METAGENOMIC_DATA$name)],2),
						round(METAGENOMIC_DATA$f_match[1],2)))

			GREP_F_MATCH <- ifelse(!is.na(SOURMASH_SPECIES) & any(grepl(SOURMASH_SPECIES, METAGENOMIC_DATA$name)), TRUE,
					ifelse(!is.na(SOURMASH_GENUS) & any(grepl(SOURMASH_GENUS, METAGENOMIC_DATA$name)), TRUE,
						FALSE))

			METAGENOMIC_ORGANISM_WITH_HIGHEST_PROBABILITY <- ifelse(!is.na(SOURMASH_SPECIES) & any(grepl(SOURMASH_SPECIES, METAGENOMIC_DATA$name)), NA,
					ifelse(!is.na(SOURMASH_GENUS) & any(grepl(SOURMASH_GENUS, METAGENOMIC_DATA$name)), NA,
						str_remove(toString(strsplit(toString(METAGENOMIC_DATA$name[1]), ' ',)[[1]][2:3]), ',')))
		} else {
...
`r if(METAGENOMIC_DATA_NOT_EMPTY) {
	if (GREP_F_MATCH) {
		sprintf("Probability for this organism is **%s** (Score is from 0 to 1; 1 means exact match).\n", F_MATCH_VALUE)
	} else {
		printf("Organism with the highest probability based on the metagenomic analysis is: **%s** with a value of **%s** (Score is from 0 to 1; 1 means exact match).\n",
