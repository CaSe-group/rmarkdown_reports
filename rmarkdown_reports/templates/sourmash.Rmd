...
Sourmash is a command-line tool and Python library for comparison of DNA sequences and plotting the results. Further it 
includes the function of "k-mer" (DNA-strings of length k) based taxonomic exploration as well as classification routines for genome 
and metagenome analysis, which are utilized here. Therefore a "Genome Taxonomy Database" (GTDB), provided by sourmash, is used for
classification, which contains ~ 258k genomes.
...
**Table 1: Taxonomic classification of sample %s.**\n
...
`r if(METAGENOMIC_DATA_EXIST & GREP_F_MATCH) {
	sprintf("Probability for this organism is **%s**.\n", F_MATCH_VALUE)
...
`r if (METAGENOMIC_DATA_EXIST) {
	sprintf("#### **Distribution of found organisms**\n\nThe following diagramm gives a overview about the distribution of all found
	organisms in the provided DNA-sequences. It is based on the approximate number of base pairs in common between the DNA-sequence(s)
	to classify and the accordingly matched organism in the database (\"intersect_bp\").  
...
`r if (METAGENOMIC_DATA_EXIST) {
	sprintf("**Figure 1: Distribution of all found organisms in sample %s.** The distribution is based on the approximate number of
		base pairs in common between the queried DNA-sequences and the matched organism.\n\n", SAMPLE_NAME)
}`

#### **Citation**

Upon usage of this tool cite sourmash as follows:
